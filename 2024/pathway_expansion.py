...
def expand_complex_into_pairwise_interactions(df,comp_col,geneList):
    ...
    if len(array) > 0:
        ...
        new_df = pd.DataFrame(columns=['GOI','partner'])
    return new_df
...
def query_BioGRID(geneList):
    ...
    params = {
        ...
        'geneList': '|'.join(geneList), 'start':0, 'max':10000
    }
    
    df_array = []
    r = rs.get(url,params=params)
    if r.status_code==200:
        df1 = pd.read_json(r.content).transpose()
        i = 1
        while df1.shape[0]>0:
            df_array.append(df1)
            params.update({'start':10000*i})
            r = rs.get(url,params=params)
            if r.status_code==200:
                df1 = pd.read_json(r.content).transpose()
                i += 1
        ...
        #filtering criteria
        df = df.query('EXPERIMENTAL_SYSTEM_TYPE == "physical"').copy()
...
def query_IntAct(geneList):
    ...
    params = {
        ...
        'pageSize': 500, 'page': 0, 'negativeFilter': 'POSITIVE_ONLY',
    }
    df_array = []
    '''
    #gene by gene query
    ...
    if r.status_code==200:
        ...
        for p in range(1,(page_num+1)):
            ...
            if r.status_code==200:
                ...
                print('Query failed on page %d with status code: %d' % (p,r.status_code))
                break
    else:
        print('Query failed with status code: %d' % r.status_code)
...
def query_ReactomeComplex(geneList):
    print('>>Start querying REACTOME complexes')
    url = 'https://reactome.org/ContentService/search/query'
    params = {
        'query': ','.join(geneList), 'species': 'Homo sapiens', 'types': 'Complex', \
        'cluster': 'true', 'parserType': 'STD', 'rows': 200, 'Start row':0,
    }
    r = rs.get(url,params=params)
    if r.status_code==200:
        total_num = r.json()["numberOfMatches"]
        page_num = int(total_num/200)+1
        print('Query REACTOME with %d hits in %d pages' % (total_num,page_num))
        array = []
        results = r.json()["results"][0]['entries']
        for item in results:
            array.append(pd.Series(item))
        for p in range(1,page_num):
            params.update({'Start row':p*200})
            r = rs.get(url,params=params)
            if r.status_code==200:
                results = r.json()["results"][0]['entries']
                for item in results:
                ...
                print('Query page %d failed with status code: %d' % (p,r.status_code))
                break

        df = pd.concat(array,axis=1).transpose()
        #filtered criteria
        df = df.loc[df.apply(lambda x: x['species'][0]=='Homo sapiens',axis=1),:]
        #ignore compartments for now to remove redundancy
...
def query_LIANA(geneList):
    ...
    if df.shape[0] > 0:
        ...
        df = pd.DataFrame(columns=['GOI','partner'])
...
def query_MetaCore(geneList):
    df = pd.read_csv('/mnt/efs/home/iet5740/Projects/GSP/pathway_expansion/MetaCore_processed_ligand_receptor.csv')
    #filtering for interactions containing GOIs
    df = df.query('Source == @geneList | Target == @geneList')
    if df.shape[0] > 0:
        df.loc[:,'GOI'] = df.apply(lambda x: x['Source'] if x['Source'] in geneList else x['Target'],axis=1)
        df.loc[:,'partner'] = df.apply(lambda x: x['Target'] if x['Source'] in geneList else x['Source'],axis=1)
    else:
        df = pd.DataFrame(columns=['GOI','partner'])        
    df.loc[:,'database'] = ['MetaCore'] * df.shape[0]
    return df
...
def compile_partners(geneList):
    ...
    metacore_df = query_MetaCore(geneList)
    array = []
    for df in [biogrid_df, intact_df, string_df, reactome_df, liana_df, complexportal_df, metacore_df]:
        df2 = df.loc[df['partner'].map(lambda x: not re.search(r'^[A-Z][A-Z0-9]+',x)==None),:]
        df2 = df2.loc[df2['GOI'].map(lambda x: not re.search(r'^[A-Z][A-Z0-9]+',x)==None),:]
        df2 = df2[['GOI','partner','database']].groupby('GOI').agg(lambda x: set(x)).reset_index()
