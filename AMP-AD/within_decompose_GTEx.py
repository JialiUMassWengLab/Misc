...
def patientCluster(df0,module_gene_dir,prefix):
    ...
    clustNum = {'glia8':10, 'glia14':3, 'glia18':8}
    array = []
    moduleGenes = {}
    sns.set(context='talk',font_scale=1.6)
    from matplotlib.backends.backend_pdf import PdfPages
    pdf = PdfPages(prefix + '.patientCorrCluster2.pdf')
    for module in ['glia8','glia14','glia18']:
        geneList = list(pd.read_csv(os.path.join('/home/jzhuang/AMP/FP_MSBB_Code',module+'.genes.txt'),header=None,index_col=0).index)
        ...
        g = sns.clustermap(dist,row_linkage=Z,col_linkage=Z,vmin=.75,row_colors=colors,colors_ratio=0.015,figsize=(30,25))
...
def plotScatter(df0,moduleGenes,clusterDf,prefix):
    ...
    for module in ['glia8','glia14','glia18']:
        ...
        df['cluster'] = df.apply(lambda x: 'cluster2' if not re.search(r'_',x.name) else 'cluster1',axis=1)
        df = df.pivot(index='gene',columns='cluster',values='coef')
        #df.iloc[1,1] = df.iloc[0,0]
        df['deviation'] = abs(df['cluster1'])
        df['cluster1'] = df['cluster1'] + df['cluster2']
        df = pd.concat([df,names],join='inner',axis=1)
        df = df.sort_values('deviation',ascending=False)

        fig,ax = plt.subplots(figsize=(15,12))
        sns.scatterplot(x='cluster1',y='cluster2',data=df,color='black',linewidth=2,s=120,ax=ax)
        ax.axline((0,0),slope=1,color='red',linestyle='--',alpha=.8)
        ax.set_xlabel('cluster A coef (log2)')
        ax.set_ylabel('cluster B coef (log2)')
        #top = df.iloc[:20,:]
        top = df[list(map(lambda x: x in ['C1QA','C1QB','C1QC','FCGBP','FCGR3A','CD14','TYMP','HAMP'],df['Description']))]
        for label,x,y in zip(top['Description'],top['cluster1'],top['cluster2']):
            plt.annotate(label,xy=(x,y),xytext=(5,0),textcoords='offset points',ha='left',va='center',fontsize=12,style='italic',fontweight='bold',color='darkred')
        plt.title(module)
...
def selectRepGenes(df,moduleGenes):
    df = df.loc[:,map(lambda x: x in moduleGenes,df.columns)]
...
def main():
    ...
    df = pd.read_csv(fn_prefix+'.csv.gz',low_memory=False,index_col=0)
    df.columns = map(lambda x: re.sub(r'^X','',x),df.columns)
    df.index = map(lambda x: re.sub(r'\.\d+$','',x),df.index)
    df['biotype'] = df.index.map(biotypes)
    df = df[list(map(lambda x: x in analysis_types,df['biotype']))]
    df = df.iloc[:,:-1].apply(lambda x: x/sum(x)*1000000,axis=0)
    #df = df[df.apply(lambda x: len([i for i in x if i>=1])/float(df.shape[1]) > .75,axis=1)]
    print(df.shape)
