...
def plotV(fn):
    ...
    df.columns = ['geneID','logFC','pvalue','geneName']
    ...
    df.plot('logFC','logP',kind='scatter',alpha=0.8,c=df[['logFC','logP']].apply(lambda x: 'blue' if x[1]<pv_cutoff or abs(x[0]) <= fc_cutoff else 'red',axis=1),figsize=(18,15),fontsize=30,s=150,edgecolor='white',linewidth=1.5,ax=ax)
    sig1 = df[(df['logP']>pv_cutoff) & (abs(df['logFC'])>fc_cutoff)]
    #sig = df[(df['logP']>pv_cutoff2) & (abs(df['logFC'])>fc_cutoff2)]
    geneList = ['C1QA','C1QB','C1QC','FCGR3A','FCGBP','CD14','TYMP','HAMP']
    sig = sig1.loc[map(lambda x: x in geneList,sig1['geneName'])]
    print(sig1.shape)

    #for label,x,y in zip(sig['geneName'],sig['logFC'],sig['logP']):
    #    plt.annotate(label,xy=(x,y),xytext=(2,0),textcoords='offset points',ha='left',va='center',style='italic',fontweight='bold',color='darkred')
    #texts = []
    #for label,x,y in zip(sig['geneName'],sig['logFC'],sig['logP']):
    ...
    plt.xlabel('log2 FoldChange',fontsize=40,fontweight='bold')
    plt.ylabel('-log10 adjusted P-value',fontsize=40,fontweight='bold')
    plt.xlim(-8,8)
