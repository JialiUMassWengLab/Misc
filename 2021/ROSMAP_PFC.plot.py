...
def main():
    ...
    sc.pl.umap(adata, color='new_cluster', legend_loc='on data', size=12, ax=ax)
    plt.savefig('ROSMAP_PFC.umap.pdf',format='pdf')

    marker_genes = ['AQP4','GAD1','MOG','FYB','CLDN5','PDGFRA','SLC17A7','CUX2']
    fig,axes = plt.subplots(2,4,figsize=(32,20))
    for i,gene in enumerate(marker_genes):
        sc.pl.violin(adata, gene, groupby='new_cluster', ax=axes[int(i/4)][i%4], rotation=40, size=2)
        labels = axes[int(i/4)][i%4].get_xticklabels()
        axes[int(i/4)][i%4].set_xticklabels(labels,ha='right')
    plt.tight_layout()
